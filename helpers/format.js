...
module.exports = (obj, notifications, linter) => {
    if ( obj !== Object(obj) ) {
        return '';
    }

    // Result array that will be used to generate output
    const result = [chalk.reset(' ')];
    ...
    const columns = Object.keys(obj.files).reduce((seq, key) => {
        const file = obj.files[key];

        file.messages.forEach((message) => {
            const positionLength = `${message.line}:${message.column}`.length;
    ...
    let descriptionMaxLength = Math.max(0, stdOutColumns - columns.position - columns.severity - columns.rule - (Object.keys(columns).length + 1));
    if ( !descriptionMaxLength || isNaN(descriptionMaxLength) ) {
        descriptionMaxLength = 80;
    }

    // Parse messages that were passed via `obj`
    Object.keys(obj.files).forEach((key) => {
        const file = obj.files[key];
        ...
        file.messages.forEach((message) => {
            const severityLine = message.severity === labels.ERROR
            ...
            const description = message.description.match(new RegExp(`.{1,${descriptionMaxLength}}`, 'g'));
